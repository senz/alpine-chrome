[![GitHub Stars](https://img.shields.io/github/stars/senz/alpine-chrome)](https://github.com/senz/alpine-chrome/) [![Docker Build Status](https://img.shields.io/github/actions/workflow/status/senz/alpine-chrome/build.yml)](https://github.com/senz/alpine-chrome/actions/workflows/build.yml)
...
- 📦 Tiniest Headless Chrome (Compressed size: [423 MB](https://github.com/senz/alpine-chrome#image-disk-size))
...
- 👷‍♂️ Autobuild with [GitHub Container Registry](https://github.com/senz/alpine-chrome/pkgs/container/alpine-chrome) to sync the project and ship the images with confidence
- 📌 Up-to-date latest Chromium and tags available to test different versions of Chromium
- 🔐 Secure with the best way to use Chrome and Docker - [See "3 ways to securely use Chrome Headless"](https://github.com/senz/alpine-chrome#3-ways-to-securely-use-chrome-headless-with-this-image) section
- 🌐 Ready for internationalisation use: support for asian characters - [See "screenshot-asia.js" file](https://github.com/senz/alpine-chrome/blob/master/with-puppeteer/src/screenshot-asia.js)
- 💄 Ready for design use: support for WebGL, support for emojis- See ["How to use with WebGL" section](https://github.com/senz/alpine-chrome#how-to-use-with-webgl) and ["Emojis are not rendered properly" section](https://github.com/senz/alpine-chrome/issues/114)
- 📄 Open Source with an Apache2 licence
- 💚 Dev-friendly with examples using NodeJS, Puppeteer, docker-compose and also a test with a X11 display - [See "Run examples" section](https://github.com/senz/alpine-chrome#run-examples)

# 3 ways to securely use Chrome Headless with this image

## ❌ With nothing

Launching the container using only `docker container run -it ghcr.io/senz/alpine-chrome ...` will fail with some logs similar to [#33](https://github.com/senz/alpine-chrome/issues/33).
...
`docker container run -it --rm ghcr.io/senz/alpine-chrome` and use the `--no-sandbox` flag for all your commands.
...
Launch the container using:
`docker container run -it --rm --cap-add=SYS_ADMIN ghcr.io/senz/alpine-chrome`
...
[chrome.json](https://github.com/senz/alpine-chrome/blob/master/chrome.json)

Also available here `wget https://raw.githubusercontent.com/jfrazelle/dotfiles/master/etc/docker/seccomp/chrome.json`

Launch the container using:
`docker container run -it --rm --security-opt seccomp=$(pwd)/chrome.json ghcr.io/senz/alpine-chrome`
...
You can change the `CHROMIUM_FLAGS` by overriding the environment variable using: `docker container run -it --rm --env CHROMIUM_FLAGS="--other-flag ..." ghcr.io/senz/alpine-chrome chromium-browser ...`

You can get full control by overriding the entrypoint using: `docker container run -it --rm --entrypoint "" ghcr.io/senz/alpine-chrome chromium-browser ...`

## Use the devtools

Command (with no-sandbox): `docker container run -d -p 9222:9222 ghcr.io/senz/alpine-chrome --no-sandbox --remote-debugging-address=0.0.0.0 --remote-debugging-port=9222 https://www.chromestatus.com/`
...
Command (with no-sandbox): `docker container run -it --rm ghcr.io/senz/alpine-chrome --no-sandbox --dump-dom https://www.chromestatus.com/`

## Print a PDF

Command (with no-sandbox): `docker container run -it --rm -v $(pwd):/usr/src/app ghcr.io/senz/alpine-chrome --no-sandbox --print-to-pdf --hide-scrollbars https://www.chromestatus.com/`

## Take a screenshot

Command (with no-sandbox): `docker container run -it --rm -v $(pwd):/usr/src/app ghcr.io/senz/alpine-chrome --no-sandbox --screenshot --hide-scrollbars https://www.chromestatus.com/`

### Size of a standard letterhead.

Command (with no-sandbox): `docker container run -it --rm -v $(pwd):/usr/src/app ghcr.io/senz/alpine-chrome --no-sandbox --screenshot --hide-scrollbars --window-size=1280,1696 https://www.chromestatus.com/`

### Nexus 5x

Command (with no-sandbox): `docker container run -it --rm -v $(pwd):/usr/src/app ghcr.io/senz/alpine-chrome --no-sandbox --screenshot --hide-scrollbars --window-size=412,732 https://www.chromestatus.com/`

### Screenshot owned by current user (by default the file is owned by the container user)

Command (with no-sandbox): `` docker container run -u `id -u $USER` -it --rm -v $(pwd):/usr/src/app ghcr.io/senz/alpine-chrome --no-sandbox --screenshot --hide-scrollbars --window-size=412,732 https://www.chromestatus.com/ ``
...
```shell
docker image build -t ghcr.io/senz/alpine-chrome:with-deno-sample .
```

Then launch the container:

```shell
docker container run -it --rm ghcr.io/senz/alpine-chrome:with-deno-sample
...
```shell
docker container run -it --rm -v $(pwd):/usr/src/app ghcr.io/senz/alpine-chrome:with-deno-sample run helloworld.ts
...
```shell
docker container run -it --rm -v $(pwd)/src:/usr/src/app/src --cap-add=SYS_ADMIN ghcr.io/senz/alpine-chrome:with-puppeteer node src/pdf.js
```
...
```shell
docker container run -it --rm -v $(pwd)/with-puppeteer/test:/usr/src/app/test --cap-add=SYS_ADMIN ghcr.io/senz/alpine-chrome:with-puppeteer node test/screenshot-asia.js
```
...
```shell
docker container run -it --rm -v $(pwd)/src:/usr/src/app/src --cap-add=SYS_ADMIN ghcr.io/senz/alpine-chrome:with-puppeteer-xvfb node src/extension.js
```
...
```shell
docker container run -it --rm -v $(pwd)/test:/usr/src/app/test --cap-add=SYS_ADMIN ghcr.io/senz/alpine-chrome:with-playwright node test/test.js
```
...
`docker container run -it --rm --cap-add=SYS_ADMIN -v $(pwd):/usr/src/app ghcr.io/senz/alpine-chrome --screenshot --hide-scrollbars https://webglfundamentals.org/webgl/webgl-fundamentals.html`

`docker container run -it --rm --cap-add=SYS_ADMIN -v $(pwd):/usr/src/app ghcr.io/senz/alpine-chrome --screenshot --hide-scrollbars https://browserleaks.com/webgl`
...
```shell
docker container run -it --rm --cap-add=SYS_ADMIN  -p 4444:4444 ghcr.io/senz/alpine-chrome:with-selenoid -capture-driver-logs
```
...
```shell
docker container run --rm -it --entrypoint "" --user root ghcr.io/senz/alpine-chrome sh
```
...
- 🐳 [docker-compose](https://github.com/senz/alpine-chrome/blob/master/examples/docker-compose) to launch a chrome calling a nginx server in the same docker-compose
- ☸️ [kubernetes](https://github.com/senz/alpine-chrome/tree/master/examples/k8s) to launch a pod with a headless chrome
- 🖥 [x11](https://github.com/senz/alpine-chrome/blob/master/examples/x11) to experiment this image with a X11 server.
...
- Where to file issues: https://github.com/senz/alpine-chrome/issues
...
```shell
docker container run -it --rm --entrypoint "" ghcr.io/senz/alpine-chrome cat /etc/alpine-release
# 3.19.1
```
...
```shell
docker container run -it --rm --entrypoint "" ghcr.io/senz/alpine-chrome chromium-browser --version
...
```shell
docker image inspect ghcr.io/senz/alpine-chrome --format='{{.Size}}'
# 663644797 # 633 MB
```
